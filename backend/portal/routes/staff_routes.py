...
Two invariants hold whatever the client later asks for:
...
def create_staff():
    ...
    if payload.get("confirm_password") is not None and payload["confirm_password"] != password:
    ...
    audit(
        ...
        entity="user",
        entity_id=user.id,
        detail=f"Created {role_name} account for {name}",
    )
    db.session.commit()
    dashboard_changed("staff_created")

    return success(_serialise(user), message=f"{name} added", status=201)
...
def update_staff(user_id):
    ...
    # Optional: only reset when a new one is actually supplied.
    if payload.get("password"):
        if len(payload["password"]) < MIN_PASSWORD:
            return error(f"Password must be at least {MIN_PASSWORD} characters", status=422)
        if (
            payload.get("confirm_password") is not None
